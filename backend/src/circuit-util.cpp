...
Circuit seq(const Circuit& first, const Circuit& second) {
  ...
  for (auto assn : second_copy.get_assignments()){
	if (!assn.get_const_inputs().empty())
      	result.add_assignment(assn.get_output().get_name(), assn.get_op(), assn.get_const_inputs());
  }
...
Circuit par(const Circuit& a, const Circuit& b) {
  ...
  for (auto a_assn : a1.get_assignments()){
    if (!a_assn.get_inputs().empty())
      result.add_assignment(a_assn.get_output().get_name(), a_assn.get_op(),
                            a_assn.get_inputs());
    if (!a_assn.get_const_inputs().empty())
      result.add_assignment(a_assn.get_output().get_name(), a_assn.get_op(), a_assn.get_const_inputs());
  }

  for (auto b_assn : b1.get_assignments()){
    if (!b_assn.get_inputs().empty())
      result.add_assignment(b_assn.get_output().get_name(), b_assn.get_op(),
                            b_assn.get_inputs());
    if (!b_assn.get_const_inputs().empty())
      result.add_assignment(b_assn.get_output().get_name(), b_assn.get_op(), b_assn.get_const_inputs());
  }
